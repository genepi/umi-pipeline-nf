...
def parse_args(argv):
    ...
    parser.add_argument(
        ...
        dest="USE_CONTEXT",
        action="store_true",
        help="Use a context primer sequence to locate the UMIs"
    )
...
def extract_umi(query_seq, query_qual, pattern, max_edit_dist, format, direction):
    ...
    if direction == "fwd":
        ...
        clip_position = umi_end_pos

    if format == "fastq":
        umi_qual = query_qual[umi_start_pos:umi_end_pos]

    return edit_dist, umi_seq, umi_qual, clip_position
    ...
    with pysam.FastxFile(input_file) as fh, open(output_file, "w") as out:
        for entry in fh:
            ...
            if use_context:
                ...
                # Extract rev UMI
                result_3p_rev_umi_dist, result_3p_rev_umi_seq, result_3p_rev_umi_qual, umi_start_rev = extract_umi_context(
